...
function assignEnvs(customEvnVariables: object, nodeEnvObject: Object) {
  ...
  Object.keys(flattedObject).forEach(variable => {
    if (nodeEnvObject[flattedObject[variable]] !== undefined) {
      ...
      envVariableMissing.push(flattedObject[variable]);
    }
  });
  if (envVariableMissing.length > 0) {
    throw new Error(`Following env variable are missing: \n${envVariableMissing.join('\n')}`);
  }
...
const configInitialized = () => {
  ...
  let constants: DeepPartial<IConfigApp> = defaultConstants;
